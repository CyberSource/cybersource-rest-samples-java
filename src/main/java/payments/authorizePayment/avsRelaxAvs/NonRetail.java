...
public class NonRetail {
   ...
   private CreatePaymentRequest getRequest(){
		...
		V2paymentsProcessingInformationCaptureOptions options=new V2paymentsProcessingInformationCaptureOptions();
		//model to be fixed
		/*options.ignoreAvsResult("Y");
		options.ignoreCvResult("N");*/
		

		V2paymentsProcessingInformation processingInformation = new V2paymentsProcessingInformation();
		processingInformation.captureOptions(options);
	...
	public NonRetail() throws Exception {
		process();
	}
